...
The WASM boundary uses the same length-and-area default as `Parser::default()`.
The `expected_dimension` argument replaces that registry with the named
domains and applies the same set as a hard output filter. Several domains are
written as a comma-separated list (`"length,area"`); an empty string keeps the
minimal default. Readings with no dimension — such as dates and bare numbers —
remain available. An unrecognized member is dropped without taking recognized
members with it. If *every* supplied name is unrecognized — a bare `"lenght"`
— the call is refused with `REJECTED_BY_POLICY` rather than silently falling
back to another registry. The array-returning exports report that refusal as
one match spanning the whole input, since an empty array would be the same
silence. The unrestricted catalog is deliberately not an implicit WASM export;
name every domain a boundary needs.
